...
FileSystem::FileSystem(bool format)
{
    ...
    if (format)
    {
        ...
        // Brute-force reset the disk
        char *buf[SectorSize];
        memset(buf, 0, SectorSize*sizeof(char));
        for(int i=0; i<NumSectors; i++){
            kernel->synchDisk->WriteSector(i, (char*)buf);
        }
...
// MP4
int FileSystem::Create(char *name, int size)
{
    Directory *directory;
    OpenFile *f;
    ...
    DEBUG(dbgFile, "Creating file " << name << " size " << size);

    vector<char *> path_list = SplitPath(name);

    int dir_sector = TraverseDirectory(path_list, DirectorySector, 0);
    f = new OpenFile(dir_sector);
    directory = new Directory(NumDirEntries);
    directory->FetchFrom(f);

    if (directory->Find(path_list[path_list.size()-1]) != -1)
    ...
    else
    {
        ...
        else if (!directory->Add(path_list[path_list.size()-1], sector))
        ...
        else
        {
            ...
            else
            {
                success = TRUE;
                // MP4
                hdr->SetIsDir(FALSE); // marked as a file
...
OpenFile * FileSystem::Open(char *name)
{
    ...
    sector = directory->Find(path_list[path_list.size()-1]);
...
bool FileSystem::Remove(char *name)
{
    ...
    directory->Remove(path_list[path_list.size()-1]);
...
void FileSystem::recursion(vector<char*> path, int parent, int level){
    ...
    for(int i=0; i<directory->getTableSize(); i++){
        ...
        if(entry->inUse){
            // cout << "entry sector = "<< entry->sector << endl;
            // if(entry->sector >= 1024) continue;
    ...
    if (directory->Find(path_list[path_list.size()-1]) != -1) {
        ...
        else
        {
            ...
            else
            {
                success = TRUE;
                // MP4
                hdr->SetIsDir(TRUE); // marked as a directory
                // everthing worked, flush all changes back to disk
                hdr->WriteBack(sector);
                directory->WriteBack(f);
                freeMap->WriteBack(freeMapFile);
...
vector<char *> FileSystem::SplitPath(char *path){
    vector<char *> res;
    char *p = new char[256];
    strcpy(p, path);
