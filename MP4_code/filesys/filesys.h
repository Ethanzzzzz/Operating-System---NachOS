...
public:
	...
	OpenFileId OpenForFileId(char *path);
	...
	void recursion(vector<char*> path, int parent, int level);

	bool CreateDirectory(char *name);

	vector<char*> SplitPath(char *path);

	int TraverseDirectory(vector<char*> path, int parent, int level);

	OpenFile *openfile;

	// TraversePath(vector<char*> path_list);

	// vector< pair<int, OpenFile*> > OpenFileTable;
