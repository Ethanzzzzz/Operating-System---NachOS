...
void Thread::setStatus(ThreadStatus st) {
    ...
    else if(st == BLOCKED){ // running to waiting -> calculate apx, and reset CPUBurstTime
        CPUBurstTime += kernel->stats->totalTicks - cacheBurstTime;
        DEBUG(dbgScheduler, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << getID() << "] update approximate burst time, from: [" << apxBurstTime << "], add [" << CPUBurstTime << "], to [" << 0.5 * CPUBurstTime + 0.5 * apxBurstTime << "]");
