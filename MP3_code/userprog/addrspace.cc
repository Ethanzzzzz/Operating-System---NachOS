...
AddrSpace::~AddrSpace() {
    ...
    for(int i = 0; i < numPages; i++){
        kernel->frameTable[pageTable[i].physicalPage] = 0;
    }
    kernel->numRemainFrame += numPages;
    delete pageTable;
}
...
bool AddrSpace::Load(char *fileName) {
    ...
    // start code
    // cout << "numPage = " << numPages << endl;
    if(numPages > kernel->numRemainFrame){
    ...
    for (int i = 0; i < numPages; i++) {
        ...
        for(int f = 0; f < NumPhysPages; f++){
            if(kernel->frameTable[f] == 0){
                pageTable[i].physicalPage = f; // assign the fth frame to this page
    ...
    int remainSize, fileOffset, pageOffset, readSize;
    ...
    if (noffH.code.size > 0) {
        ...
        fileOffset = noffH.code.inFileAddr;
        pageOffset = noffH.code.virtualAddr % PageSize;
        readSize = min(PageSize - pageOffset, remainSize);
        vAddr = noffH.code.virtualAddr;
        while(remainSize > 0){
            this->Translate(vAddr, &pAddr, 0);
            executable->ReadAt(
            ...
            fileOffset += readSize;
            remainSize -= readSize;
            readSize = min(PageSize, remainSize);
        }
    }
    if (noffH.initData.size > 0) {
        ...
        fileOffset = noffH.initData.inFileAddr;
    ...
    if (noffH.readonlyData.size > 0) {
        ...
        fileOffset = noffH.readonlyData.inFileAddr;
        pageOffset = noffH.readonlyData.virtualAddr % PageSize;
        readSize = min(PageSize - pageOffset, remainSize);
